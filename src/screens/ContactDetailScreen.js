...
import { makePhoneCall, navigateToLocation, sendSMS } from '../utils/communication';
import { formatLastContact, formatDateTime } from '../utils/time';
...
const ContactDetailScreen = ({ route, navigation }) => {
  ...
  const handleNavigate = async () => {
    if (contact.latitude && contact.longitude) {
      ...
      if (!success) {
        Alert.alert('导航失败', '请检查是否安装了导航应用');
      }
    } else {
      Alert.alert('位置信息缺失', '该联系人暂无位置信息');
    }
  };
    ...
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.statusRow}>
          ...
          {renderStars(contact.relationship)}
        </View>
      ...
      <View style={styles.section}>
        ...
        <View style={styles.infoCard}>
          ...
          <View style={styles.infoItem}>
            <View style={styles.infoLeft}>
              <Icon name="road" size={16} color="#10b981" />
              <Text style={styles.infoLabel}>距离</Text>
            </View>
            <Text style={styles.infoValue}>{contact.distance}{contact.distanceUnit}</Text>
          </View>
          ...
          <View style={styles.infoItem}>
            <View style={styles.infoLeft}>
              ...
              <Text style={styles.infoLabel}>最近联系</Text>
            </View>
            <Text style={styles.infoValue}>{formatLastContact(contact.lastContact)}</Text>
          </View>
          ...
          <View style={styles.infoItem}>
            <View style={styles.infoLeft}>
              <Icon name="comment-dots" size={16} color="#8b5cf6" />
              <Text style={styles.infoLabel}>状态</Text>
            </View>
            <Text style={styles.infoValue}>{contact.context}</Text>
          </View>
          ...
          <View style={styles.infoItem}>
            <View style={styles.infoLeft}>
              <Icon name="calendar-check" size={16} color="#ec4899" />
              <Text style={styles.infoLabel}>最佳联系时间</Text>
            </View>
            <Text style={styles.infoValue}>{contact.bestTime}</Text>
          </View>
        </View>
      </View>
        ...
        <View style={styles.tagsCard}>
          {contact.categories.map((cat, index) => (
            <View key={index} style={styles.categoryTag}>
              <Icon
                name={cat.dim === 'work' ? 'briefcase' : 'user-friends'}
                size={12}
                color={cat.dim === 'work' ? '#3b82f6' : '#10b981'}
              />
              <Text style={styles.categoryTagText}>{cat.path.join(' > ')}</Text>
            </View>
          ))}
        </View>
      </View>
      ...
      <View style={styles.section}>
        <TouchableOpacity style={styles.editButton} onPress={handleEdit}>
          ...
          <Text style={styles.editButtonText}>编辑联系人</Text>
        </TouchableOpacity>
